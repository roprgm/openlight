...
| **Healing patch** | One painted, non-destructive repair in a Healing layer. Its algorithm is Smart clone (nearby donor with bounded boundary correction) or AI Remove (a generated crop stored as a document image resource). Patches replay in order and each sees the result of the preceding patches. Size belongs to the recorded hard stroke; feather and opacity remain editable on its accumulated shape. |
