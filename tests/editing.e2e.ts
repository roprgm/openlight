...
test("edit a photo, inspect the preview and histograms, undo changes, and export", async ({
	...
	await test.step("color mixer switches channels, groups vertical drags, exports selected colors and resets", async () => {
		...
		await page.keyboard.press("ControlOrMeta+Shift+z");
		await expect(saturation).toHaveValue("0");
		const bytes = await readFile("tests/fixtures/photo.svg");
		await page.evaluate(
			async (bytes) => {
				await window.openlight.loadImage(
					new File([new Uint8Array(bytes)], "photo.svg", {
						type: "image/svg+xml",
					}),
				);
			},
		...
		await expect.poll(async () => (await state()).history.undoCount).toBe(0);
	});
