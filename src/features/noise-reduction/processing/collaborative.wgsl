...
struct Params { origin: vec2i, size: vec2i, stage: u32, bayer: u32, shot: vec4f, read: vec4f, variance: array<vec4f, 16> }
...
// RGB arrives in opponent channels; linear Bayer planes use an orthonormal Hadamard basis.
fn color(v: vec4f) -> vec4f {
  if params.bayer == 0u { return v; }
  let sums = vec2f(v.x + v.y, v.z + v.w);
  let differences = vec2f(v.x - v.y, v.z - v.w);
  return 0.5 * vec4f(sums.x + sums.y, differences.x + differences.y, sums.x - sums.y, differences.x - differences.y);
}
// RGB uses measured perceptual variances; Bayer uses signal-dependent sensor variances.
fn noiseVariance(signal: vec4f) -> vec4f {
  if params.bayer != 0u { return max(params.read + params.shot * max(signal, vec4f(0.0)), vec4f(1e-10)); }
...
fn main(@builtin(workgroup_id) group: vec3u, @builtin(local_invocation_index) lane: u32) {
  ...
  if params.stage == 1u {
    for (var g = 0u; g < 8u; g++) {
      data[g * 64u + lane] = vec4f(0.0);
      if g < count { data[g * 64u + lane] = color(loadGuide(matches[g] + pixel)); }
    }
  }
  workgroupBarrier();
  ...
  groups(lane);
  spatial(lane, true);
  let weight = window(lane % 8u) * window(lane / 8u) / max(energies[0], vec4f(select(1e-10, dot(params.read, vec4f(0.25)), params.bayer != 0u)));
  for (var g = 0u; g < 8u; g++) {
