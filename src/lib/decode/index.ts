...
export type { Decoded, Pixels } from "./types";

/** Decoder backed by a worker module: post the file, receive transferred pixels or an error. */
function workerDecoder(load: () => Promise<{ default: new () => Worker }>) {
...
const heic = workerDecoder(() => import("./heic.worker?worker"));
...
/** Matches by MIME type, then by extension for files the OS doesn't type (RAW, HEIC on Windows). */
...
export default async function decode(file: File) {
	const format = formatOf(file);
	if (!format) {
		throw new Error(`Unsupported image: ${file.name}`);
	}
	return (await format.load())(file);
}
