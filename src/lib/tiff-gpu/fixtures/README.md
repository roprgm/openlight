...
Small synthetic TIFF files, one per feature, written once with tifffile. `reference.json` holds the output size and, for a few pixels of each, the linear Rec.2020 `rgba` values tiff-gpu must produce, computed from standard sRGB and ICC color math rather than from the shaders.
