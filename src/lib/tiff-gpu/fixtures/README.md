...
Small synthetic TIFF files, one per feature, written once with tifffile. `reference.json` holds, for a few pixels of each, the 16-bit `samples` tiff-gpu must produce and the linear Rec.2020 `rgba` OpenLight converts them to, computed from standard sRGB and ICC color math rather than from the shaders.
