...
- `lib/decode` converts every format into the working space. Browser-decoded formats go through `linearize`. TIFF uses `lib/tiff-gpu`, which returns a linear Rec.2020 target on its own: `prepareTiff` (directory, color profile, CPU codecs) runs in the worker and `uploadTiff` does the rest on the GPU. `lib/camera-raw` builds on it for camera raw: `prepareDng` finds the Bayer mosaic and its camera tags, `developDng` demosaics, balances, and converts on the GPU. Each format entry in `lib/decode` names its GPU leg; browser-decoded formats default to `linearize`. Reuse `workerDecoder` for decoders to come (16-bit PNG, RAW).
