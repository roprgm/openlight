...
Layer composition builds subgraphs without allocating textures for containers. `mixAdjustment` combines an effect with its input using opacity and optional analytical gradients in document coordinates; Add/Subtract child masks use a small storage buffer in that same pass, without mask textures; it preserves input alpha. Node names include stable layer IDs. Removed layers release their graph-owned instance handles and storage buffers through `release(prefix)`; bypassed layers keep their instances. vgpu's internal pipeline and uniform caches remain GPU-owned until GPU disposal.
