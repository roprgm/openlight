...
A mask that paints with a brush is rasterized instead: [`createMaskRaster`](src/core/renderer/mask/index.ts) keeps one `r8unorm` texture per such layer at source resolution, outside the graph. Its own gradient or strokes and its child masks apply in stored order as blended passes; stroke dabs stamp in scissored chunks of 64, one frame each so buffer writes stay ordered. When a scene only appends strokes or points to the last brush, only the new dabs stamp; any earlier change rebuilds the texture from the strokes. The mix pass then samples that texture under the name `layer/<id>/raster` rather than `layer/<id>/mix`, and the display overlay samples the same texture. `inspect()` reports `stamped` dabs and the live `rasters`. A mask with no coverage is bypassed without a texture.
