...
- Use `Notice` for a message floating over the viewport that must not block editing, such as a storage or renderer failure. Choose its `tone` (status or alert, which sets the ARIA role) and `placement`, and pass `onDismiss` when the user may close it.
