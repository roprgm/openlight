...
Features that are not ready for everyone are behind [Vercel Flags](https://vercel.com/docs/flags). `api/flags.ts` evaluates them on the deployment, where the project's credentials live, and the app reads that endpoint once at startup to compose an enabled feature; nothing else reads a flag. The development server serves the same function, using the credentials `vercel env pull` writes to `.env.local`; without them every flag is off. Browser tests fulfil `/api/flags` themselves.

| Flag | Enables |
| --- | --- |
| `ai-heal` | The AI Remove algorithm in Healing, with its local inference runtime. Off by default while it is unstable on iOS. |
